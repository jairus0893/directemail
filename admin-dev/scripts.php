...
var gds = false;
var popup = false;
var support = '<?php    echo $_SESSION['support'];  ?>';
...
function removeexclusion(eid)
{
    $.ajax({
        ...
        success: function(resp){
        	...
        	alert("Exclusion list deleted successfully.");
        	listMenu('manageexclusion');
        }
    });
}
function idleprojectid(nonauto) {
    if($("#idleprojectid").prop('checked') == true){
    	clearTimeout($refreshinfo);
    	document.getElementById('loading').style.display = 'inline';
	} else {
		$refreshinfo = setTimeout("refreshinfo()",10000);
		document.getElementById('loading').style.display = 'inline';
	}
}
function refreshinfo(nonauto)
{
    ...
    $("#idleprojectid").attr('disabled', false);
    if (apt.includes("Live Monitor")) {
        $.ajax({
            url: "live.php?act=refresh",
            success: function(resp){
                var obj = JSON.parse(resp);
                if($('input[name=idleprojectid]').is(':checked')) {
			        $.ajax({
			            url: "live.php?act=showidlecampaign&sub=showidle",
			            success: function(resp1){
			                var obj1 = JSON.parse(resp1);
			                $("#activecamp").html(obj1.activecamp);
...
function refreshagentstat(nonauto)
{
    var fil2 =  $("#liveuserstable_filter input").val();
    var lsorting = $("#liveusers table").dataTable().fnSettings().aaSorting;;
    var ldisp = $("#liveusers table").dataTable().fnSettings()._iDisplayLength;
    ...
    if (apt.includes("Live Monitor"))
        {
            $.ajax({
            url: "live.php?act=refreshagentstat",
            success: function(resp){
                var obj = JSON.parse(resp);
                ...
                if (!nonauto) {
                    setTimeout("refreshagentstat()",5000);
                }
            }
            });
    }
...
<?php    include("agentspy/scripts-include.php")   ?>
...
{
	 jQuery('#jqdialog').html("<iframe src='../modules/appbook.php?ci="+ci+"&cid="+clientid+"' width='100%' height='100%'  frameborder='0'></iframe>");
...
function togglednclist(dncid, a)
{
    ...
    $.ajax({
        url: "admin.php?act=updatednclist&dncid="+dncid+"&field=dnc_active&val="+val
    });
	...
	navbar.addButton([<?php    if (checkrights('livemonitor')) { 
			...
			<?php    if (checkrights('manage_list')) { 
                          ?>new Ext.Toolbar.MenuButton({cls:'x-btn-text-icon', icon: 'icons/lists.png',
							...
							clickEvent: 'click', 
							menu: [
						{cls:'x-btn-text-icon', icon: 'icons/listsnew.png', text: 'New List', handler: clickhandle, action: 'newlist'},{cls:'x-btn-text-icon', icon: 'icons/listsnew.png', text: 'New Exclusion List', handler: clickhandle, action: 'newexclusionlist'},<?php    /*{cls:'x-btn-text-icon', icon: 'icons/listsnew.png', text: 'CSV Update', handler: clickhandle, action: 'listupdatefile'},
      ...
      if ($features->voip && checkrights('manage_campaign'))
      {
			...
			new Ext.Toolbar.MenuButton({cls:'x-btn-text-icon', icon: 'icons/chat.png',
							text: 'Chat',  clickEvent: 'click', menu: [
									...
									{cls:'x-btn-text-icon', icon: 'icons/chatlogs.png', text: 'Chat Logs', handler: clickhandle, action: 'chatlog'}]}),<?php  }  ?>
...
function clickhandle(linker)
	{
        ...
        else if (linker.action == 'exportcampaignlists')
	        {
	            dialogwindow(linker.action);
	        }
...
function listMenu(action)
{
   ...
   if (action == 'managedonotcall')
    {
        $.ajax({
                    url:'admin.php?act=getapp&app=managedonotcall',
                    type:'GET',
                    global: false,
                    success: function(resp)
                    {
			...
			success: function(data)
                                {
                                    if (test == false)
                                        ...
                                        $.ajax({
                                            url: '../interface/emailer.php?act=sendemail&tid='+templateid+'&uid=<?php    echo $_SESSION['auth'];  ?>',
                                            type: 'POST',
                                            data: sdata,
...
function getinfo()
	{
        $.ajax({
            ...
            success: function(resp){
                ...
                dynawidth();
                $refreshinfo = setTimeout("refreshinfo()",10000);
                $refreshagentinfo = setTimeout("refreshagentstat()",5000);
            }
        });
	}
function dynawidth()
...
function dialogwindowml(action,lid)
{
	jQuery.ajax({
        url: "admin.php?act=getapp&app="+action+"&lid="+lid,
        success: dresponsehandler
    });
}
...
function exclusionupload() {
  var xhr = new XMLHttpRequest();
  var fd = new FormData(document.getElementById('exclusionuploadcsv'));
...
function dispoupdateupload() {
  var xhr = new XMLHttpRequest();
  var fd = new FormData(document.getElementById('dispoupdateuploadcsv'));
...
function statusoptions()
{
	...
	if (sttype.substring(0,8) == 'transfer')
        ...
        else if (sttype == 'link' || sttype == 'externalpost')
            {
...
function indicator()
{
    ...
    $.ajax({
        url: "../listener.php?act=admincheckin&stamp=<?php    echo $_SESSION['logid'];  ?>&sid=<?php    echo session_id();  ?>&_timestamp="+_timestamp.getTime(),
        global: false,
                timeout: 20000,
        success:    function(resp){
...
function removednc(dncid)
{
	$.ajax({
	    url: 'admin.php?act=removednc&id='+dncid,
	    success: function(resp){
