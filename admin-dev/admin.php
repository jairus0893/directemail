...
if ($act == 'getstatusbypid')
{
	...
	{
		// $pid = 0;
		$res = mysql_query("select statusname from statuses where ( (projectid in (SELECT projectid FROM projects where bcid=$bcid and active=1)) $GLOBAL_DEFAULT_DISPO_CONDITION ) GROUP BY statusname");
	}
	else 
	{
		$res = mysql_query("select statusname from statuses where ( (projectid in (SELECT projectid FROM projects where projectid=$pid)) $GLOBAL_DEFAULT_DISPO_CONDITION ) GROUP BY statusname");
	}
...
include('customfields/savecf.php');
...
if ($act == 'incpace') {
	...
	$getbcclients = mysql_query("select maxdialpace from bc_clients where bcid = ".$row['bcid']."");
	$getbcclients_row = mysql_fetch_row($getbcclients);
	if ($pace < $getbcclients_row[0]) {
		$np = intval($pace);
		$np++;
...
if ($act == 'updatednclist')
	{
		$f = $_REQUEST['field'];
		$l = $_REQUEST['dncid'];
		$v = $_REQUEST['val'];
		mysql_query("update donotcall_list set $f = '".mysql_real_escape_string($v)."' where id = '$l'");
		exit;
	}
if ($act == 'javaexport')
	{
...
if ($act == 'manreport')
	{
     ...
     $headers[] = 'Actions';
     $headers[] = 'Delete';
	 $headers[] = 'Last Update'; /* ADDED BY Vincent Castro */
		foreach ($reps as $rep)
			{
				if ($rep['status'] == 'hold')
					{
	...
	$date = new DateTime($rep['date']); /* ADDED BY Vincent Castro */
	$rows[$rep['reportid']]['data'] = $date->format('M d, Y H:i:s'); /* ADDED BY Vincent Castro */
			}
...
if ($act == 'export')
	{
		// include "export.php";
		include "listexport.php";
	}
if ($act == 'exclutionlistexport') {
	$id = $_REQUEST["id"];
	$disp = "";
...
if ($act == 'donotcallexport') {
	$dncid = $_REQUEST["dncid"];
	$disp = "";
	$donotcallres = mysql_query("SELECT phone from donotcall where dncid = ".$dncid."");
    while ($donotcallrow = mysql_fetch_assoc($donotcallres)) {
        $disp .= $donotcallrow["phone"]."\r\n";
    }
    header("Content-type: application/x-msdownload");
    header("Content-Disposition: attachment; filename=washlistupdate_".$dncid.".csv");
    header("Pragma: no-cache");
    header("Expires: 0");
    echo $disp;
    exit;
	...
	$projrow = mysql_fetch_array($projres);
        ...
        if ($cf)
        {
            ...
            foreach ($cf as $key=>$value)
            {
                ...
                $cflist .= '<td class="cf-label" id="cf-label-'.str_replace(" ", "", $key).'">'.$value.'</td>';
                $cflist .= '<td class="cf-label-editable" id="cf-label-editable-'.str_replace(" ", "", $key).'" style="display:none;"><input type="text" value="'.$value.'" name="'.$key.'"> <a href="#" id="cf-save" onclick="savecf(\''.$projid.'\',\''. addcslashes(htmlspecialchars(str_replace(" ", "", $key)),"'").'\')"> Save </a></td>';
                $cflist .= '<td>';
                $cflist .= '<a href="#" class="cf-cancel" id="cf-cancel-'.str_replace(" ", "", $key).'" style="display:none;" onclick="cancelcf(\''.str_replace(" ", "", $key).'\')"> Cancel </a>';
                $cflist .= '<a href="#" class="cf-edit" id="cf-edit-'.str_replace(" ", "", $key).'" onclick="editcf(\''.str_replace(" ", "", $key).'\')"> Edit </a> | ';
			...
			<div id="campsettingsdisplay" style="float:left;width:82%;" >
                <div id="pdetailsdiv" class="campsection">
                    ...
                    <h3>Dial Settings</h3>
												...
												$getbcclientsdialpace = mysql_query("select maxdialpace from bc_clients where bcid = ".$projrow['bcid']."");
                								$getbcclientsdialpace_row = mysql_fetch_row($getbcclientsdialpace);
												for ($i = 0; $i <= $getbcclientsdialpace_row[0]; $i++) {
                                                    $selected = '';
                                                    if ($i == $projrow['dialpace']) $selected = 'selected="selected"';
                                                    $newdpdrop .= '<option value="'.$i.'" '.$selected.'>'.$i.'</option>';
                                                }
                                                $rows[3][2] = '<select id="dialpace'.$projrow['projectid'].'" onchange="mc_update(\'dialpace\',\''.$projrow['projectid'].'\')">'.$newdpdrop.'</select>';
...
include("timetracker_management/ttEventsOptShowSelect.php");
                    ...
                    <?php
                                                ...
                                                $rows[4][1] = 'SLA (seconds)';
                                                $rows[4][2] = '<input type="text" id="inboundsla'.$projrow['projectid'].'" onblur="mc_update(\'inboundsla\',\''.$projrow['projectid'].'\')" value="' .$projrow['inboundsla']. '">';
                                                //$rows[4][1] = 'Dial Prefix';
                                                //$rows[4][2] = ' <input type="text" value="'.$projrow['prefix'].'" onblur="mc_update(\'prefix\',\''.$projrow['projectid'].'\')" id="prefix'.$projrow['projectid'].'" />';
                                                 echo tablegen($headers,$rows,'100%');
                                        }
include("agentinterface_management/agentuisettings.php");
include("timezone/timezonesettings.php");
...
if ($act == 'addstatus')
	...
	mysql_query("INSERT into statuses set userid = '$uid', timestamp = '$datenow', projectid = '$pid', statusname = '".mysql_real_escape_string($statusname)."', statustype = '$statustype', category = '$category', dispocat = '$dispocat', options='".mysql_real_escape_string($options)."'");
