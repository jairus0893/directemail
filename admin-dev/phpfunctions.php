...
function savelead($leadid)
{
            ...
            $dres = mysql_query("SELECT * from statuses where statusname = '".$_POST['dispo']."' and projectid ='$pid'");
			   ...
			   if ($tocat == 'agent' || $tocat == 'team')
			   {
				   ...
				   {
					   mysql_query("INSERT into statuses set userid = ".$uid.", timestamp = '".$datenow."', projectid = '".$nlist['projects']."', statusname = '".$data['dispo']."', statustype = 'text', category = '$tocat'");
				   }
			   }
   ...
   function saveleadforqa($leadid,$client_disposition)
	   {
			   $record = new records($leadid);
			   //mysql_query("update dateandtime set dtime = '".$_POST['dtime']."' where leadid = '$leadid' ");
			   $record->setdatetime($_POST['dtime']);
			   ...
			   if ($client_disposition != $record->data['cdisposition']) {
				$ccsrecres = mysql_query("SELECT * FROM client_statuses WHERE client_statusid = '".$record->data['cdisposition']."'");
		        $ccsrecord = mysql_fetch_assoc($ccsrecres);
	            $record->addnote("System","Disposed by ".$_SESSION['username']." as ".$ccsrecord["statusname"]);    
			}
			if ($_POST['dispo'] != NULL)
            {
				$record->addnote("System","Disposed by ".$_SESSION['username']." as ".$_POST['dispo']);    
			}
            if ($dotransfer > 0) {
                $torow = $drow;           
            $data = $record->data;
...
function checkexistinglistnotdeleted($table,$field,$value)
{
    $res = mysql_query("SELECT $field from $table where $field = '$value' and is_deleted = 0");
    $count = mysql_num_rows($res);
    return $count;
}
?>