...
function loadtab(tab,bcid)
{
    if (tab == 'extensions') {
        $.ajax({
            ...
            url: "super.php?act=clientextensions&bcid="+bcid,
            success: function(resp) {
                        $("#tabextensions").html(resp);
                        $("#tabextensions table").dataTable();
...
function activetoggle()
{
    ...
    if (vl == 'Active') {
    	...
    	window.location = 'super.php?inactive';
    } else if (vl == 'Deactivated') {
    	window.location = 'super.php?deactivated';
    }
}
...
$(document).ready(function(e) {
 ...
 <?php
  ...
  $(".dataTables_length").html('<select id="bulkaction" onchange="extbulkdeleteproviders()"><option value="">Bulk Action</option><option value="delete">Delete</option></select>');
  $(".dataTables_info").html(" ");
  $(".jbut").button();
 ...
 ?>
     dtable = $(".clientstable").dataTable();
      if (dtable)
          {
            $(".dataTables_filter").html('View: <select id="clientabletoggle" onchange="activetoggle()"><option value="Active">Active</option><option value="Inactive" <?=$inactiveselected;?> >Inactive</option><option value="Deactivated" <?=$deactivatedselected;?> >Deactivated</option></select>');
