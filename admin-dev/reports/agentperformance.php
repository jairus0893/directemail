...
$dres = mysql_query("select statusname from statuses where ( (projectid in (SELECT projectid FROM projects where bcid=$bcid and active=1)) $GLOBAL_DEFAULT_DISPO_CONDITION ) GROUP BY statusname");
...
if ($act == 'view')
	{
		...
		if (is_array($multimo)) {
			foreach ($multimo as $mmo) {
				$mmo_arr[] = "'".$mmo."'";
			}
			$mmo_str = implode(",",$mmo_arr);
		} else {			
			$mmo_str = "'".$multimo."'";
		}
		if ($mo == 'none') {
			...
			$mmo_q = " and agentdisposition in ($mmo_str) ";
		}
		...
		while ($r = mysql_fetch_array($res))
			{
                ...
                $actionsquery = "SELECT userid, action, sum(epochend - epochstart) as actionduration from actionlog where epochend > 0 and epochstart >= '".strtotime($start)."' and epochstart <= '".strtotime($end." 24:59:59")."' AND epochend IS NOT NULL AND ((epochend - epochstart) < 28800) and $projquery group by userid, action";

phplog("actionsquery: " . $actionsquery);
...
phplog("agents: " . json_encode($agents));
		...
		$res = mysql_query("SELECT userid,startepoch, endepoch, answeredtime, dialedtime, dialmode,agentdisposition from finalhistory where startepoch >= '".strtotime($start)."' and startepoch <= '".strtotime($end." 24:59:59")."' and $projquery");
                while ($r = mysql_fetch_assoc($res))
			{
                            ...
                            {
                                if ($r['dialedtime'] == 0) 
                                    $dialedtime = $r['endepoch'] - $r['startepoch'];
                                else 
                                    $dialedtime = $r['dialedtime'] - $r['answeredtime'];
                    ...
                    while ($r = mysql_fetch_array($res))
			{
                ...
                $qt = "select userid, count(*) as contacts from finalhistory where startepoch >= '".strtotime($start)."' and startepoch <= '".strtotime($end." 24:59:59")."' and $projquery and userid != 0 $mmo_q group by userid;";
                $res = mysql_query($qt);
		while ($r = mysql_fetch_array($res))
			{
		...
		$totalagenttar = array();
		$totalagentcount = 0;
		$totalagentcontacts = 0;
		foreach ($agents as $agent) {

phplog("AGENT: " . json_encode($agent));
			...
			if ($agent['userid'] > 0) {
                                ...
                                // $realdial = $agent['dialedtime'] + abs( $agent['dial'] - ($agent['dialedtime']+$agent['realtalk']) );
                                $realdial = $agent['dial'] - $agent['dialedtime'];
...
phplog("totaltalk (dial+talk): " . $totaltalk);
phplog("realtalk (realtalk): " . $realtalk);
phplog("realdial (dialedtime): " . $realdial);

				//For Total Tally
				$totalagentrealtalk += $agent['realtalk'];
				// $totalagentdialedtime += $agent['dialedtime'];
				$totalagentdialedtime += $realdial;
				$totalagentpause += $agent['pause'];
				$totalagentwrap += $agent['wrap'];
				$totalagentwait += $agent['wait'];
				$totalagentpreview += $agent['preview'];
				$totalagentcount += $agent['callcount'];
				$totalagentcontacts += $agent['contacts'];
                ...
                foreach ($objectives->agents as $cobj) {
                    $totalagentobj['obj-'.$cobj['id']] += $agent['obj-'.$cobj['id']];
                    $totalagenttar['tar-'.$cobj['id']] += $agent['tar-'.$cobj['id']];
            ...
            $totaltemptalk = number_format($totalagentrealtalk /60,2,".",'');
            $totaltempdial = number_format($totalagentdialedtime /60,2,".",'');
            $totaltemppause = number_format($totalagentpause /60,2,".",'');
            $totaltempwrap = number_format($totalagentwrap /60,2,".",'');
            $totaltempwait = number_format($totalagentwait /60,2,".",'');
            $totalprevtime = number_format($totalagentpreview /60,2,".",'');
            $totalchours = $totaltemptalk + $totaltempdial + $totaltemppause + $totaltempwait + $totaltempwrap + $totalprevtime;
            $totaltemphours = number_format($totalchours /60,2,".",'');
            $table .= '<tr>
                        <td class="'.$class.'"></td>
                        <td class="'.$class.'"><b>Total</b></td>
                        <td class="'.$class.'"><b>'.$totaltemphours.'</b></td>
                        <td class="'.$class.'"><b>'.$totaltemptalk.'</b></td>
                        <td class="'.$class.'"><b>'.nuform($totaltemptalk,$totalchours).'% </b></td>
                        <td class="'.$class.'"><b>'.$totaltempdial.'</b></td>
                        <td class="'.$class.'"><b>'.nuform($totaltempdial,$totalchours).'% </b></td>
                        <td class="'.$class.'"><b>'.$totaltempwrap.'</b></td>
                        <td class="'.$class.'"><b>'.nuform($totaltempwrap,$totalchours).'% </b></td>
                        <td class="'.$class.'"><b>'.$totaltempwait.'</b></td>
                        <td class="'.$class.'"><b>'.nuform($totaltempwait,$totalchours).'% </b></td>
                        <td class="'.$class.'"><b>'.$totaltemppause.'</b></td>
                        <td class="'.$class.'"><b>'.nuform($totaltemppause,$totalchours).'% </b></td>
                        <td class="'.$class.'"><b>'.$totalprevtime.'</b></td>
                        <td class="'.$class.'"><b>'.nuform($totalprevtime,$totalchours).'% </b></td>
                        <td class="'.$class.'"><b>'.$totalagentcount.'</b></td>';
                        foreach ($objectives->agents as $cobj) {
                            $table .= '<td class="'.$class.'"><b>'.$totalagentobj['obj-'.$cobj['id']].'</b></td>';
                            $table .= '<td class="'.$class.'"><b>'.$totalagenttar['tar-'.$cobj['id']].'</b></td>';
                        }
                        if ($mo != 'none') {
                            $table .= '<td class="'.$class.'"><b>'.$totalagentcontacts.'</b></td>';
                            $table .= '<td class="'.$class.'"><b>'.nuform($totalagentcontacts,$totalagentcount).'% </b></td>';
                        }
            $table .= '</tr>';    
		if ($act2 == 'excel')
			{
...
<table width="929" border="0" cellspacing="0" cellpadding="5">
  ...
  <tr>
    ...
    <td><select name="mo[]" id="mo" multiple="multiple">
...
function viewrep()
{
    ...
    //var mo = $("#mo").val();
    var mo = $('#mo :selected').map(function(){
  		return this.value
	}).get();
	$.ajax({
