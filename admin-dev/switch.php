...
if ($act == 'removednc') {
    $id 		= $_REQUEST['id'];
	$dnclistres = mysql_query("SELECT donotcall_list.*, donotcall.phone, lists.listid as listname FROM donotcall_list 
				JOIN donotcall ON donotcall.dncid = donotcall_list.id 
				JOIN lists ON lists.lid = donotcall_list.dnc_listid
				WHERE donotcall_list.id = '".$id."'");
	while ($dnclistrow = mysql_fetch_assoc($dnclistres)) {
		$listid = $dnclistrow["listname"];
		mysql_query("UPDATE lists SET washdate = '".$dnclistrow["oldwashdate"]."' WHERE lid = '".$dnclistrow["dnc_listid"]."'");
		mysql_query("UPDATE donotcall_list set dnc_active = 0, dnc_is_deleted = 1 WHERE id = '".$id."'");
	}
	$weblogres = mysql_query("SELECT leadid, oldvalue, listid FROM weblog WHERE dncid = '".$id."' AND field = 'dispo' AND listid = '".$listid."'");
	while ($weblogrow = mysql_fetch_assoc($weblogres)) {
		mysql_query("UPDATE leads_raw SET dispo = '".$weblogrow["oldvalue"]."' WHERE leadid = '".$weblogrow["leadid"]."' AND listid = '".$weblogrow["listid"]."'");
		mysql_query("UPDATE leads_done SET dispo = '".$weblogrow["oldvalue"]."' WHERE leadid = '".$weblogrow["leadid"]."' AND listid = '".$weblogrow["listid"]."'");
...
if ($act == 'voicemail' || $act == 'deletevm')
{
	...
	?>
	<script>
	function vmuserlogs(phonenumber, datetime, userid) {
		var projid = <?=$pid;?>;
		$.ajax({
			url: "admin.php?act=vmuserlogs&projid="+projid+"&phonenumber="+phonenumber+"&datetime="+datetime+"&userid="+userid,
    ...
    <table width="800" cellspacing="5">
        ...
        <tr><th>Date/Time</th><th>CallerID</th><th>Size</th><th>Checked By</th><th>File</th></tr>		
        <?php
		$cta = 0;
                krsort($audiosort);
		foreach ($audiosort as $aud)
			{
				...
				if ($m = 0) $cl = "tableitem";
								...
								$vmlogrow = mysql_query("SELECT userid FROM vmlog WHERE projectid = $pid and phone_number = '$fphone' and epoch = '$dt' ORDER BY timestamp DESC");
								$vmlogres = mysql_fetch_assoc($vmlogrow);
								$urow = mysql_query("SELECT afirst, alast FROM memberdetails WHERE userid = ".$vmlogres["userid"]."");
								$ures = mysql_fetch_assoc($urow);
				echo '<tr class="'.$cl.'" id="'.$dt.'"><td>'.$dti.'</td>
				<td>'.$fphone.'</td><td>'.$lc.'</td><td>'.$ures["afirst"].' '.$ures["alast"].'</td>';
    ...
    <param name="FlashVars" value="mp3='.urlencode($media).'&amp;bgcolor=c7c7c7&amp;slidercolor=404040" />
                                ...
                                //echo '</td>'
								echo '<td><a id="wavfile" href="'.$media.'" target="_blank"><span onclick="vmuserlogs(\''.$fphone.'\', '.$dt.', '.$_SESSION['uid'].')">Download</span></a> | <a href="#" id="deletevm" onclick="deletevm(\''.$aud['name'].'\',\''.$pid.'\',\''.$dt.'\')">Delete</a></td></tr>';
								$cta++;
			}
		?>
        </table>
...
if ($act == 'getapp')
	{
		...
		switch ($app) {
                        ...
                        case 'dispoupdate':
                            ...
                            break;
						case 'managedonotcall':
                            include "managedonotcall.php";
                            break;
			case 'chatlog':
                ...
                <div id='chathistorynote'><font color="red"><p>Note: Chat history is purged daily at 08:30pm (GMT+8)</p></font></div>                
                <?
...
<?				break;
    ...
    <?	
			...
			case 'newlist':
            	...
            	break;
			case 'exportcampaignlists':
                checkrights('managelists');
				$lid = $_REQUEST['lid'];
				echo "";
				include "exportcampaign/exportlistpercampaign.php";
				break;
			case 'editcampaignlists':
                checkrights('managelists');
				$lid = $_REQUEST['lid'];
				echo "";
				include "editcampaignlists/editcampaignlistdialog.php";
				break;
