...
if ($act == 'updateext')
	{
		extract($_POST);
		mysql_query("UPDATE bc_phones set name = '$username', defaultuser='$username', secret = '$secret', classification = '$classification',bcid= '$bcid' where name = '$extname'");
...
if ($act == 'editext')
	{
        ...
        <tr><td>Classification</td><td>
    		<select name="classification" id="classification">
    			<option value="'.$ext['classification'].'">'.$ext['classification'].'</option>
...
foreach ($clients as $client)
        ...
        include("superadmin/superswitch-include.php");
	exit;
}
if ($act == 'partners')
{
...
if ($act == 'clientextensions') {
    $extres = mysql_query("select * from bc_phones where bcid = '".$_REQUEST['bcid']."'");
    while ($row = mysql_fetch_assoc($extres))
    {
    ...
    $header[]='Used By';
	$header[]='Last Used';
	$header[]='How Many Days Last Used';
	$header[]='Action';
	foreach ($extensions as $ext) {
		$currentfirstdate = date('Y-m-01', strtotime(date("Y-m-d")));
		$currentlastdate = date('Y-m-t', strtotime(date("Y-m-d")));
		$getcallhistoryextres = mysql_query("select * from callhistory where 
							exten = '".$ext['name']."' and bcid = '".$_REQUEST['bcid']."' AND 
							startepoch >= '".strtotime($currentfirstdate. " 00:00:00")."' AND 
							startepoch <='".strtotime($currentlastdate. " 23:59:59")."'
							ORDER BY startepoch DESC LIMIT 1");
		$getcallhistoryextrow = mysql_fetch_assoc($getcallhistoryextres);
		
	    $rows[$ext['name']]['1'] = '<input type="checkbox" name="bulkaction" value="'.$ext['name'].'">';
		$rows[$ext['name']]['2'] = $ext['name'];
		$rows[$ext['name']]['3'] = $ext['secret'];
		$rows[$ext['name']]['4'] = $ext['classification'];
		$rows[$ext['name']]['5'] = $clients[$ext['bcid']]['company'];
	    if ($liveusers[$ext['name']]) {
	    	$rows[$ext['name']]['6'] = $liveusers[$ext['name']]['afirst'].' '.$liveusers[$ext['name']]['alast'];
	    } else {
	    	$rows[$ext['name']]['6'] = 'Not In use';
	    }
		if ($getcallhistoryextrow["startepoch"] != NULL) {
			$rows[$ext['name']]['7'] = date("Y-m-d H:i:s", $getcallhistoryextrow["startepoch"]);	
		} else {
			$rows[$ext['name']]['7'] = "";
		}
		if ($getcallhistoryextrow["startepoch"] != "" || $getcallhistoryextrow["startepoch"] != NULL) {
			$date1 = strtotime(date("Y-m-d"));
			$date2 = $getcallhistoryextrow["startepoch"];
			$timeDiff = abs($date1 - $date2);
			$numberDays = $timeDiff/86400;
			$numberDays = intval($numberDays);
		    $rows[$ext['name']]['8'] = $numberDays;	
		} else {
			$rows[$ext['name']]['8'] = "";
		}
		$rows[$ext['name']]['9'] = '<a href="super.php?act=delext&ext='.$ext['name'].'">Delete</a> | <a href="#" onclick="editext(\''.$ext['name'].'\',\''.$_REQUEST['sub'].'\')">Edit</a> | <a href="#" onclick="agentspyinextension(\''.$ext['name'].'\', \'whisper\')">Whisper</a>';
	}
		...
		foreach ($extensions as $ext)
			{
				...
				$rows[$ext['name']]['4'] = $ext['classification'];
				$rows[$ext['name']]['5'] = $clients[$ext['bcid']]['company'];
                if ($liveusers[$ext['name']]) $rows[$ext['name']]['6'] = $liveusers[$ext['name']]['afirst'].' '.$liveusers[$ext['name']]['alast'];
                else $rows[$ext['name']]['6'] = 'Not In use';
				$rows[$ext['name']]['7'] = '<a href="super.php?act=delext&ext='.$ext['name'].'">Delete</a> | <a href="#" onclick="editext(\''.$ext['name'].'\',\''.$_REQUEST['sub'].'\')">Edit</a> | <a href="#" onclick="bargethis(\''.$ext['name'].'\')">Barge</a>';
			}
...
<tr><td><b>Account Status: </b></td><td>
<select name="status" id="status" onchange="triggerchange();clientchange(this)">
<?php
if ($client['status'] == 'Active') {
    ?>
    <option value="<?php echo $client['status']; ?>" selected="selected"><?php echo $client['status']; ?></option>
