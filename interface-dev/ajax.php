...
include_once '../PHPAGI/ast_remote_control.php';

include "phpfunctions.php";
require_once '../classes-dev/classes.php';
include "../ami-client.php";
include "queuepreview/ajax-include.php";
...
if ($act == 'newlead')
{
    ...
    <?php
        if ($_REQUEST['dialmode'] == 'inbound')
        {
            ?>
            <input type="button" value=" New Entry " onclick="createnewinboundentry('NO NAME')" id="skipinbound"/>
            <?php
        }
        else
        {
            ?>
            <input type="button" value=" New Entry " onclick="createnewentry('NO NAME')" id="skipoutbound"/>
            <?php
        }
    ?>
        </div>
...
if ($act == 'inbounduselead')
{
    ...
    $record->data['callid'] = $pidrow['callid'];
     echo json_encode($record->data);
     exit;
...
if ($act == 'nlsearch')
	{
		...
		$resdone =  mysql_query("SELECT leads_done.*, projects.projectname from leads_done 
								...
								where (leadid like '%".$nlphone."%' or phone like '%".$nlphone."%' or cname like '%".$nlphone."%' or cfname like '%".$nlphone."%' or company like '%".$nlphone."%' or clname like '%".$nlphone."%' or altphone like '%".$nlphone."%') and leads_done.projectid in ($projectidlist)");
    ...
    where (leadid like '%".$nlphone."%' or phone like '%".$nlphone."%' or cname like '%".$nlphone."%' or cfname like '%".$nlphone."%' or company like '%".$nlphone."%' or clname like '%".$nlphone."%' or altphone like '%".$nlphone."%') and leads_raw.listid in ($listidlists)";
...
if ($act == 'startrecording')
{
    ...
    $recordfile = "/var/spool/asterisk/monitor/$bcid/$pid/" . $pidrow['leadid'] . "_" . time() . "_OPREC.wav";
    $_res = "NULL";
    if ($pidrow['recording'] == 0 && $pidrow['leadid'] > 0) 
    {
        $_res = ast_confbridge_startrecording($pidrow['confserver'], $conference, $recordfile);

        if ( preg_match("/Recording started/", $_res["data"]) )
        {
            $_liveusers_res = mysql_query("update liveusers set recording = 1 where extension = '$conference'", $GLOBAL_MYSQL_CONNECTION_ID);
            if (!$_liveusers_res)
            {
                $_res = mysql_error($GLOBAL_MYSQL_CONNECTION_ID);
            }
            else 
            {
                $_res = "Success";
    ...
    echo json_encode( array ("ACT" => $act, "recording" => $pidrow['recording'], "conference" => $conference, "recordfile" => $recordfile, "leadid" => $pidrow['leadid'], "result" => $_res) );
    exit;
}
if ($act == 'stoprecording')
{
    $conference = $pidrow['extension'];
    $_res = "NULL";
    if ($pidrow['recording'] == 1) 
    {
        $_res = ast_confbridge_stoprecording($pidrow['confserver'], $conference);

        if ( preg_match("/Recording stopped/", $_res["data"]) )
        {
            $_liveusers_res = mysql_query("update liveusers set recording = 0 where extension = '$conference'", $GLOBAL_MYSQL_CONNECTION_ID);
            if (!$_liveusers_res)
            {
                $_res = mysql_error($GLOBAL_MYSQL_CONNECTION_ID);
            }
...
if ($act == 'dopause')
{
	...
	{
		startlog('pause');
		mysql_query("update liveusers set status = 'paused', statustimestamp = '".time()."' where userid ='$auid'");
		echo "paused";	
		exit;
	}
...
if ($act == 'doactive')
{
	...
	{
		startlog('wait');
		mysql_query("update liveusers set status = 'available', statustimestamp = '".time()."' where userid ='".$row['userid']."'");
...
if ($act == 'next')
{
    ...
    {
        mysql_query("update liveusers set status='available', webstatus='free', callid= NULL, callerid= NULL, waiting= NOW(), statustimestamp = '".time()."' where userid ='$auid'");
        echo 'checkforcalls';
        startlog('wait');
...
if ($act == 'checkzip')
{
	...
	if ($ss > 1)
	{
        ...
        echo '<input type="text" id="city" name="city" value="'.$suburb.'"  class="box" > ';
	exit;
}
if ($act == 'getinfo')
{
    ...
    {
        if ($leadid == 0 || $callerid == 'anonymous' || $pidrow['callchannel'] == NULL)
        {
	...
	if ($mandial ==1)
	{
		if (!$preview)
		{
            ...
            $region = $prefrow[1];
            $prefbcid = $prefrow[2];
            mysql_query("Update liveusers set leadid = '".$record->leadid."', status = 'dialing', actionid = '0', statustimestamp = '".time()."' where userid ='$auid' and status != 'dialing'");
		...
		{
            startlog('preview');
			mysql_query("Update liveusers set leadid = '".$record->leadid."', status = 'preview', statustimestamp = '".time()."' where userid ='$auid' and status != 'dialing'");
		}
    }
...
if ($act == 'switchdopause') {
	$data = $_GET;
	if ($data['lid']) {
		savelead($data);
	}
	$ami->hangupcall($data['lid'], $data["projectid"]);
	startlog('pause');
	mysql_query("update liveusers set status = 'paused', statustimestamp = '".time()."' where userid ='$auid'");
	echo "paused";
	exit;
}
if ($act == 'externalpostaddnote')
{
    $u = new members($auid);
    $leadid = $_REQUEST['leadid'];
    $message = $_REQUEST['message'];
    $externalpost = $_REQUEST['externalpost'];
    if ($leadid > 0) {
	    $record = new records($leadid);
	    $prevnote = $record->notes();
	    $newnote = "Lead data push to ".$externalpost." by ".$_SESSION['username']." : ".$message."";
	    $jnote = json_decode($prevnote,true);
	    if (!$jnote) $jnote = array();
	    array_push($jnote,array(
	        "user"=>$u->userlogin,
