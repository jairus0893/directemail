...
function callnumber($custrow,$phone)
  {
    ...
    mysql_query("Update liveusers set leadid = '".$custrow['leadid']."', status = 'dialing', actionid = '0', statustimestamp = '".time()."' where userid ='$auid'");
  }
...
function selectproject($u)
{
  ...
  ?>
    ...
    <select name="selectprojectid" id="selectprojectid" title="<?php echo $u;?>">
    <?php echo getprojectlist($u);?>
    </select><br>
    <?php
    if (strlen($inb) > 5)
    {
...
function savelead($data)
{
  ...
  if ($data['disposition'] == "DoNotCall" && $data['phone'] != "") {
  	$dnclistname = "agentgenerated".$pid;
    $dnclistres = mysql_query("SELECT * FROM donotcall_list WHERE dnc_listname = '".$dnclistname."' AND dnc_active = 1 AND dnc_is_deleted = 0");
	$dnclistrow = mysql_fetch_assoc($dnclistres);
    if (mysql_num_rows($dnclistres) < 1) {
    	$nlrow = mysql_fetch_assoc($nlres);
    	$pres = mysql_query("SELECT * FROM projects WHERE projectid = '".$pid."'");
    	$prow = mysql_fetch_assoc($pres);
    	$lres = mysql_query("SELECT lid FROM lists WHERE listid LIKE '".$data['list']."'");
    	$lrow = mysql_fetch_assoc($lres);
        mysql_query("INSERT INTO donotcall_list SET dnc_listname = '".$dnclistname."', dnc_listid = '".$lrow['lid']."', 
        dnc_projectid = '".$pid."', dnc_active = 1, dnc_date_created = '".time()."', dnc_bcid = '".$prow["bcid"]."'");
    	$dncid = mysql_insert_id();
    	mysql_query("UPDATE donotcall_list SET dnc_listcount = 1 WHERE id = '".$dncid."'");
		$dncres = mysql_query("SELECT * FROM donotcall JOIN donotcall_list ON donotcall_list.id = donotcall.dncid 
		WHERE donotcall_list.dnc_projectid = '".$pid."' AND donotcall.phone = '".$data["phone"]."'");
    	if (mysql_num_rows($dncres) < 1) {
			mysql_query("INSERT INTO donotcall SET dncid = '".$dncid."', phone = '".$data["phone"]."', projectid = '".$pid."'");
			mysql_query("INSERT INTO weblog SET user = '".$_SESSION['auth']."', dncid = '".$dncid."', listid = '".$data['list']."', 
						leadid = ".$data['lid'].", oldvalue = '".$data['cf1']."', newvalue = '".$data['disposition']."', field = 'dispo', epoch = unix_timestamp();");
		}
	} else {
		mysql_query("UPDATE donotcall_list SET dnc_listcount = dnc_listcount + 1 WHERE id = '".$dnclistrow["id"]."'");
		mysql_query("INSERT INTO donotcall SET dncid = '".$dnclistrow["id"]."', phone = '".$data["phone"]."', projectid = '".$pid."'");
		mysql_query("INSERT INTO weblog SET user = '".$_SESSION['auth']."', dncid = '".$dnclistrow["id"]."', listid = '".$data['list']."', 
				leadid = ".$data['lid'].", oldvalue = '".$data['cf1']."', newvalue = '".$data['disposition']."', field = 'dispo', epoch = unix_timestamp();");
	}
  }
  ...
  {  // skip if not called
    ...
    elseif ($drow['category'] == 'agent')
    {
      mysql_query("update leads_done set isteamcallback = 0 where leadid = ".$data['lid']);
    }
