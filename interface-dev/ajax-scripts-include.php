...
{
    $("#notify").block({ 
        message: '<div class="ui-state-notify ui-corner-all" style="padding: 0 .7em;"><p><span class="ui-icon ui-icon-alert" style="float: left; margin-right: .3em;"></span><strong>&nbsp;</strong><b>'+message+'</b>  Incoming Calls In Queue!</p></div>', 
        fadeIn: 700,
        fadeOut: 700, 
        timeout: 0, 
        showOverlay: false, 
...
function poll_agentqueuecheck()
{
        ...
        success: function(resp)
        {
            ...
            if (typeof resp.length === "undefined")
            {
                for (var campaign in resp)
                {
                    ...
                    $projoption.text("QUEUE: "+ resp[campaign] +' '+$projoption.attr('label'));
    ...
    setTimeout("agentqueuecheck()",random(3,10)*1000);
}
function agentqueuecheck() {
    $.ajax({
        ...
        success: function(resp)
        {
            notification(resp.queuecalls, 0, poll_agentqueuecheck);
        }
    });
}
function incomingqueuecalldetails() {
	$.ajax({
        url: "inboundqueue/agent.php?act=inboundqueuedetails&userid="+userid,
        dataType : "json", 
        success: function(resp){
        	...
        	$.each(obj, function(key, value) {
				html += "<tr style='font-size: 12px;cursor:pointer' onmouseover='cbhi(this)' onmouseout='cbout(this)'><td>"+this.projectid+"</td><td>"+this.callid+"</td><td></td><td></td></tr>";
			});
	        $("#dialogcontainer").html('<table width="100%" style="font-size:1em" class="sortable"><thead><tr><th class="tableheader">Campaign</th><th class="tableheader">Call ID</th><th class="tableheader">Phone Number</th><th class="tableheader">Waiting Time</th></tr></thead><tbody>'+html+'</tbody></table>');
	        $("#dialogcontainer").dialog({
	            title: 'Calls on Queue',
	            width: 600,
	            maxHeight: 600,
	            modal: true,
	            closeOnEscape: false,
